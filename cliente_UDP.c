...
#define SERVER_IP     "192.168.0.2" // IP del servidor
...
void socket_udp() {
    ...
    if (sendto(sock, message, strlen(message), 0, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
        ESP_LOGE(TAG, "Error al enviar el mensaje");
        close(sock);
        return;
    }
    ...
    // Recibir respuesta
    char rx_buffer[128];
    ssize_t rx_len = recvfrom(sock, rx_buffer, sizeof(rx_buffer) - 1, 0, NULL, NULL);
    if (rx_len < 0) {
